	...
	public AdminPortal() {
		...
		passText.addFocusListener(new FocusListener() {
			public void focusGained(FocusEvent e) {
				...
				System.out.println(String.valueOf(passText.getPassword()));
				if (String.valueOf(passText.getPassword()).compareTo(passMessage) == 0) {
					System.out.println("YES!!");
					passText.setText("");
			...
			public void focusLost(FocusEvent e) {
				if (String.valueOf(passText.getPassword()).compareTo("") == 0) {
					passText.setEchoChar((char) 0);
					passText.setText(passMessage);
				}
			}
	...
	class Go implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (IDstr != null) {
				...
				} catch (NumberFormatException nfe) { // If input is not a
														// number, show Error
					isValidInput = false;
					new popupMsg("Error", "Invalid Administrator ID '" + IDstr + "' entered.");
				}
